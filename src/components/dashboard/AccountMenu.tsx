...
export default function AccountMenu({ onClose, anchor }: AccountMenuProps) {
  ...
  return createPortal(
    <div
      ...
      className="fixed z-50 flex w-64 flex-col items-center rounded-2xl bg-bg-white-0 py-2 shadow-[0px_0px_10px_rgba(0,0,0,0.1)]"
    >
      <div className="flex w-full flex-col items-start px-2.5 py-2">
        ...
        <p className="text-[12px] text-text-soft-400">arthur@testhive.com</p>
      </div>
      ...
      <div className="flex w-full flex-col gap-1.5 px-2.5 py-2">
        {items.map((item) => (
          <button
            key={item.label}
            type="button"
            className="flex h-[42px] w-full items-center gap-2 rounded-lg px-3 py-2 hover:bg-bg-soft-200"
          >
            <img alt="" src={item.icon} className="size-5 shrink-0" />
            <span className="text-[14px] font-light text-text-soft-400">{item.label}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex w-full flex-col items-start px-2.5 pt-2">
        <button
          type="button"
          className="flex h-[42px] w-full items-center gap-2 rounded-lg px-2.5 py-2 hover:bg-bg-soft-200"
        >
